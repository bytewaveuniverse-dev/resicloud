        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
            ...
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 shadow-sm">
                ...
                <div class="flex flex-col">
                    ...
                    {{-- Monto en Bolívares --}}
                    <span class="text-xs font-medium text-zinc-500 counter-bs" 
                        data-target="{{ $esAdmin ? \App\Models\Asiento::totalIngresosReales()->total_bs : auth()->user()->asientos()->where('estado','pagado')->sum('monto_bs') }}">
                        Bs. 0.00
                    </span>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 shadow-sm">
                ...
                <div class="flex flex-col">
                    ...
                    {{-- Monto en Bolívares --}}
                    <span class="text-xs font-medium text-zinc-500 counter-bs" 
                        data-target="{{ \App\Models\Asiento::totalEgresos()->total_bs ?? 0 }}">
                        Bs. 0.00
                    </span>
                ...
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 shadow-sm border-b-4 border-b-green-500">
                    <div class="p-2 bg-green-100 dark:bg-green-900/30 rounded-lg text-green-600 w-fit mb-3">
                        ...
                        <div class="flex flex-col">
                            ...
                            @php $balance = \App\Models\Asiento::balanceNeto(); @endphp
                            
                            <div class="flex flex-col">
                                <span class="text-2xl font-bold text-zinc-900 dark:text-white counter" 
                                    data-target="{{ $balance['usd'] }}">
                                    $0.00
                                </span>
                                {{-- Monto en Bolívares para la Caja --}}
                                <span class="text-xs font-medium text-zinc-500 counter-bs" 
                                    data-target="{{ $balance['bs'] }}">
                                    Bs. 0.00
                                </span>
                            </div>
                            <span class="text-xs text-zinc-400 font-medium">Disponible Real</span>
...
<script>
    ...
    // --- LÓGICA PARA CONTADORES EN BOLÍVARES ---
    document.querySelectorAll('.counter-bs').forEach(counter => {
        const target = parseFloat(counter.getAttribute('data-target')) || 0;
        const duration = 1500; 
        const startTime = performance.now();
        ...
        const animate = (currentTime) => {
            ...
            const progress = Math.min(elapsed / duration, 1);
            const currentVal = progress * target;
            counter.innerText = 'Bs. ' + currentVal.toLocaleString('es-VE', {minimumFractionDigits: 2, maximumFractionDigits: 2});
            if (progress < 1) requestAnimationFrame(animate);
        };
        requestAnimationFrame(animate);
    });
